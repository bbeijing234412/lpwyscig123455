...
async function fetchPlayerRank(player) {
  ...
  const label = player.displayName || gameName;
  ...
  if (!solo) {
    return { name: label, tier: 'UNRANKED', division: null, lp: 0, wins: 0, losses: 0 };
  }
...
async function pollAll() {
  ...
  for (const player of config.players) {
    try {
      ...
      console.error(`Failed to fetch ${player.displayName || player.gameName}:`, err.message);
      results.push({
